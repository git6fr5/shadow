...
public class Mesh : MonoBehaviour {
    ...
    /* --- Static Variables --- */
    public static float PostActionDuration = 8f / 24f;
    public static float PostJumpDuration = 4f / 24f;
    public static float ScareJumpDuration = 8f / 24f;
    ...
    [SerializeField] private Sprite[] scareJump = null;
    [SerializeField] private Sprite[] scareStill = null;
    ...
    [SerializeField, ReadOnly] private float postScaredTimer = 0f;
    ...
    private void AnimateAction() {
        ...
        // Run the pre-dash
        if (actionActive) {
            if (controller.actionFlag == ActionState.Scared) {
                postScaredTimer += Time.deltaTime;
                if (postScaredTimer < ScareJumpDuration) {
                    animationData.animation = scareJump;
                    animationData.startIndex = 0;
                    animationData.length = scareJump.Length;
                    animationData.interval = ScareJumpDuration;
                }
                else {
                    animationData.animation = scareStill;
                    animationData.startIndex = 0;
                    animationData.length = scareJump.Length;
                }
            }
            else if (controller.actionFlag == ActionState.PreAction) {
